...
CAP['comercio-externo.html'] = [
  ('barra da fonte única',  lambda s: 'assets/nav-sinagepe.js' in s and 'nav-mount' in s),
  ('mostra as metas do PEDSA', lambda s: 'metasPEDSA' in s and 'function desenharMetas' in s),
  ('assinala os campos vazios', lambda s: 'class="vazio"' in s
     and 'a meta n&atilde;o pode ser medida' in s),
  ('declara que é de propósito', lambda s: 'vazio de propósito' in s
     or 'vazio de prop&oacute;sito' in s),
  ('explica porque só agregado', lambda s: 'porqueSoAgregado' in s
     and 'function desenharPorque' in s),
  ('lista as fontes a pedir', lambda s: 'fontesAPedir' in s),
  ('metas do PQG',          lambda s: 'pqg-metas' in s and 'function desenharPQG' in s),
  ('avisa crónica vs aguda',lambda s: 'avisoMetodologico' in s),
  ('comércio espelho',     lambda s: 'comercio-espelho' in s and 'function desenharEspelho' in s),
  ('registo de motores',   lambda s: 'function desenharMotores' in s),
  ('desvio não é acusação',lambda s: 'avisoObrigatorio' in s),
]
...
CAP['actualidade-dados.html'] = [
  ('catálogo de fontes',   lambda s: 'catalogo-fontes' in s and 'function desenharCatalogo' in s),
  ('diz que nenhuma é automática', lambda s: 'actualizam-se sozinhas' in s),
  ('barra da fonte única',  lambda s: 'assets/nav-sinagepe.js' in s and 'nav-mount' in s),
  ('indicadores com origem',lambda s: 'assets/indicadores-sinagepe.js' in s and 'IndicadoresSinagepe.linha' in s),
...
CAP['monitorizacao-em-falta.html'] = [
  ('estados de stock',      lambda s: 'function desenharEstadosStock' in s
     and 'estados-stock' in s),
  ('desconhecido não é zero', lambda s: 'regraFundamental' in s),
  ('barra da fonte única',  lambda s: 'assets/nav-sinagepe.js' in s and 'nav-mount' in s),
  ('indicadores com origem',lambda s: 'assets/indicadores-sinagepe.js' in s and 'IndicadoresSinagepe.linha' in s),
  # O nome passou a ser montado no Promise.all e já não aparece com extensão.
  ('lê o cadastro real',    lambda s: "'armazens-nacionais'" in s),
...
CAP['cadastro-armazens.html'] = [
  ('motor de independência', lambda s: 'independencia-fontes.js' in s
     and 'IndependenciaFontes.avaliarDivergencia' in s),
  ('marca a mesma origem',  lambda s: 'confirmações independentes' in s),
  ('série temporal da capacidade', lambda s: 'function desenharSerie' in s
     and 'infraestrutura-armazenagem-serie' in s),
  ('mostra as divergências', lambda s: 'diverg&ecirc;ncias' in s or 'divergencias' in s),
  ('barra da fonte única',  lambda s: 'assets/nav-sinagepe.js' in s and 'nav-mount' in s),
  ('indicadores com origem',lambda s: 'assets/indicadores-sinagepe.js' in s and 'IndicadoresSinagepe.linha' in s),
  # O nome do ficheiro passou a ser montado no Promise.all, e já não
  # aparece com a extensão no código.
  ('lê o cadastro real',    lambda s: "'armazens-nacionais'" in s),
...
# ---------------------------------------------------------------------------
# Paleta: as superfícies acompanham a apresentação SIGENAPE; as cores que
# codificam informação — bronze, azul, verde, âmbar, vermelho — não mudam.
# ---------------------------------------------------------------------------
def _paleta(s):
    return ('--card-bg:#141F2B' in s and '--brz:#C98A3C' in s
            and '--ok:#12B981' in s and '--red:#EF4444' in s)

for _e in ['painel-nacional.html', 'cadastro-armazens.html', 'marketplace-b2b.html',
           'comercio-externo.html', 'centro-antecipacao.html', 'indicadores-painel.html',
           'fontes-internacionais.html', 'series-precos.html', 'dashboard-banco-central.html']:
    CAP.setdefault(_e, []).append(('paleta com bronze mantido', _paleta))

CAP['assets/independencia-fontes.js'] = [
  ('conta origens, não publicações', lambda s: 'function confirmacoesIndependentes' in s
     and 'origensDistintas' in s),
  ('reconhece os produtores',  lambda s: 'PRODUTORES = [' in s and s.count("sigla: '") >= 12),
  ('avalia divergências',      lambda s: 'function avaliarDivergencia' in s
     and 'mesmaOrigem' in s),
  ('não adivinha a origem',    lambda s: 'Não adivinha a origem' in s),
  ('assinala fonte sem origem',lambda s: 'semOrigemDeclarada' in s),
]

# ---------------------------------------------------------------------------
# Cartas institucionais: as três que estão prontas para assinar.
# ---------------------------------------------------------------------------
import os as _os
def _carta(nome, exige):
    def _f(_):
        try:
            t = io.open('/mnt/user-data/outputs/docs/' + nome, encoding='utf-8').read()
        except Exception:
            return False
        return all(x in t for x in exige)
    return _f

CAP['docs/pedido-ane.md'] = [
  ('carta à ANE',    _carta('pedido-ane.md',    ['PEDSA', 'Lei n.º 34/2014'])),
  ('carta ao BAU',   _carta('pedido-bau.md',    ['PEDSA', 'egra dos cinco', 'contagem'])),
  ('carta ao SETSAN',_carta('pedido-setsan.md', ['PQG 2025-2029', 'crónica', 'Gaza'])),
  ('nenhuma promete o que o sistema não faz',
   _carta('pedido-setsan.md', ['Não classificamos insegurança alimentar'])),
]

CAP['assets/fonte-dados.js'] = [
  ('intercepta só os dados', lambda s: "data\\/([a-z0-9\\-]+)\\.json" in s
     or 'recursoDe' in s),
  ('24 recursos declarados', lambda s: s.count("'") >= 40 and 'RECURSOS = [' in s),
  ('modo api configurável',  lambda s: "CFG.modo === 'api'" in s and 'Authorization' in s),
  ('avisa que não dá segurança', lambda s: 'não torna o sistema seguro' in s.lower()
     or 'NÃO RESOLVE' in s),
  ('estado consultável',     lambda s: 'estado: function' in s),
]
...
DADOS = {
 'data/motores.json': [
   ('dez motores registados', lambda d: len(d['motores']) == 10),
   ('oito activos',        lambda d: sum(1 for m in d['motores']
      if m['estado'].startswith('activo')) >= 8),
   ('correcção registada', lambda d: 'sete dos nove' in d['meta']['correccaoRegistada']),
   ('cada motor com localização', lambda d: all(m.get('onde') or m.get('porque')
      for m in d['motores'])),
 ],
 'data/comercio-espelho.json': [
   ('endpoint público verificado', lambda d: d['fonte']['exigeChave'] is False
      and 'comtradeapi.un.org' in d['fonte']['endpointPublico']),
   ('seis parceiros com código', lambda d: len(d['parceirosPrioritarios']) == 6
      and all(p.get('codigo') for p in d['parceirosPrioritarios'])),
   ('produtos com HS',    lambda d: all(p.get('hs') for p in d['produtosEssenciaisComHS'])),
   ('desvio não é prova', lambda d: 'não é prova de infracção'
      in d['comoSeCalcula']['avisoObrigatorio']),
   ('correcção registada', lambda d: 'confundia duas coisas' in d['meta']['correccao']),
   ('quem qualifica declarado', lambda d: 'Alfândegas' in d['comoSeCalcula']['quemQualifica']),
 ],
 'data/catalogo-fontes.json': [
   ('dezasseis fontes',    lambda d: len(d['fontes']) >= 16),
   ('cada fonte com estado e periodicidade', lambda d: all(
      x.get('estado') and x.get('periodicidade') for x in d['fontes'])),
   ('zero automáticas',    lambda d: d['resumo']['comActualizacaoAutomatica'] == 0),
   ('regra de ouro',       lambda d: len(d['meta']['regraDeOuro']) == 7),
   ('declara o que exige servidor', lambda d: len(d['oQueExigeServidor']) >= 5),
   ('escala de confiança', lambda d: len(d['meta']['escalaConfianca']) == 5),
   ('todas com confiança atribuída', lambda d: all(x.get('confianca') for x in d['fontes'])),
 ],
 'data/estados-stock.json': [
   ('cinco estados de stock', lambda d: len(d['estadosDeStock']) == 5),
   ('desconhecido não é zero', lambda d: 'STOCK DESCONHECIDO NÃO É ZERO'
      in d['meta']['regraFundamental']),
   ('30 unidades em desconhecido', lambda d: any(e['estado']=='desconhecido'
      and e['quantosNoSistema'] == 30 for e in d['estadosDeStock'])),
   ('balanço não calculável', lambda d: d['balancoFisico']['calculavelHoje'] is False
      and d['balancoFisico']['quantosTemos'] == 0),
   ('avisa contra estimar sem dados', lambda d:
      'inventar com aparência de método' in d['balancoFisico']['avisoImportante']),
   ('camadas MozGIS com URL', lambda d: all(c.get('url') for c in d['camadasMozGIS']['camadas'])),
   ('avisa que são intervenções', lambda d: 'A SEREM INTERVENCIONADAS'
      in d['camadasMozGIS']['naturezaCritica']),
   ('reformulação registada', lambda d: len(d['oQueMudaNaFormulacao']['depois']) > 150),
 ],
 'data/pqg-metas.json': [
   ('quatro metas relevantes', lambda d: len(d['metasRelevantes']) == 4),
   ('meta da insegurança alimentar', lambda d: any(m['id']=='inseguranca-cronica'
      and m['de']==24 and m['para']==14 for m in d['metasRelevantes'])),
   ('distingue crónica de aguda', lambda d: any('CRÓNICA NÃO É AGUDA'
      in (m.get('avisoMetodologico') or '') for m in d['metasRelevantes'])),
   ('nenhuma medível hoje', lambda d: not any(m['sinagepePodeMedir']
      for m in d['metasRelevantes'])),
   ('cadeia ENDE-PQG-PESOE', lambda d: 'PESOE' in d['meta']['cadeia']),
   ('base legal declarada', lambda d: 'Resolução n.º 17/2025' in d['meta']['aprovacao']),
   ('críticas registadas como posição', lambda d: 'não como facto técnico'
      in d['criticasParlamentares']['nota']),
 ],
 'data/comercio-externo.json': [
   ('doze produtos essenciais', lambda d: len(d['produtosEssenciais']) == 12),
   ('cada produto com quatro componentes', lambda d: all(
      len(p['componentes']) == 4 for p in d['produtosEssenciais'])),
   ('nenhum com dependência calculável', lambda d:
      d['resumoProdutos']['comDependenciaCalculavel'] == 0),
   ('nenhum valor inventado', lambda d: all(
      c['valor'] is None for p in d['produtosEssenciais'] for c in p['componentes'])),
   ('três metas do PEDSA',  lambda d: len(d['metasPEDSA']) == 3),
   ('nenhuma medível hoje', lambda d: all(m['estado']=='sem dados' for m in d['metasPEDSA'])),
   ('componentes com fonte',lambda d: all(c.get('fonte') for m in d['metasPEDSA']
      for c in m['componentes'])),
   ('regra do campo vazio', lambda d: 'nunca que o valor é zero' in d['meta']['regra']),
   ('exclui dados individuais', lambda d: 'NUIT' in d['meta']['naoInclui']),
   ('razão técnica declarada', lambda d: 'sem servidor'
      in d['porqueSoAgregado']['razaoTecnica']),
   ('quatro fontes a pedir', lambda d: len(d['fontesAPedir']) == 4),
 ],
 'data/pedsa-enquadramento.json': [
   ('quatro pilares',      lambda d: len(d['pilares']) == 4),
   ('mandatos que sustentam o sistema', lambda d: len(d['mandatosRelevantes']) >= 4),
   ('metas com o que o sistema pode medir', lambda d: all(
      'sinagepePodeMedir' in m for m in d['metas2030'])),
   ('declara que é enquadramento, não dados', lambda d:
      d['meta']['natureza'] == 'Enquadramento institucional, não fonte de dados'),
   ('licença do MADER declarada', lambda d: 'MADER' in d['meta']['licenca']),
   ('reserva sobre o PEDSA III', lambda d: 'PEDSA III' in d['meta']['sucessao']),
   ('formulação para as cartas', lambda d: len(d['usoNasCartas']['formulacaoSugerida']) > 150),
 ],
 'data/infraestrutura-armazenagem-serie.json': [
   ('cinco pontos na série', lambda d: len(d['serie']) == 5),
   ('Zambézia com origem nos SDAE', lambda d: any(
      x.get('cadeiaOrigem',{}).get('origemPrimaria','').startswith('SDAE') for x in d['serie'])),
   ('publicador separado da origem', lambda d: any(
      x.get('orgaoPublicador') and x.get('origemDosDados') for x in d['serie'])),
   ('ICM como publicador da Zambézia', lambda d: any(
      'Instituto de Cereais' in (x.get('orgaoPublicador') or '') for x in d['serie'])),
   ('distinção declarada no meta', lambda d:
      'campos distintos' in d['meta']['distincaoPublicadorOrigem']),
   ('divergência de definição em Milange', lambda d: any(
      v.get('casoQueProva') == 'Milange' and v['razao'] >= 40 for v in d['divergencias'])),
   ('não cita repositório como fonte', lambda d: all(
      'Scribd' not in str(x.get('fonte','')) for x in d['serie'])),
   ('próximo passo declarado', lambda d: 'proximoPasso' in d
      and d['proximoPasso']['impacto'] == 5),
   ('PICA com dez províncias', lambda d: len([x for x in d['serie']
      if x['ano']==2013][0]['porProvincia']) == 10),
   ('distingue instalada de operacional', lambda d:
      'não é capacidade operacional efectiva' in d['meta']['distincaoObrigatoria']),
   ('três divergências registadas', lambda d: len(d['divergencias']) == 3),
   ('cadeia de origem por fonte', lambda d: all('cadeiaOrigem' in x for x in d['serie'])),
   ('três pontos vêm do ICM', lambda d: sum(1 for x in d['serie']
      if x.get('cadeiaOrigem',{}).get('origemPrimaria') == 'ICM') == 3),
   ('nenhuma divergência é independente', lambda d: not any(
      v.get('fontesIndependentes') for v in d['divergencias'])),
   ('regra da independência declarada', lambda d:
      'não são confirmação independente' in d['meta']['independenciaDasFontes']['regra']),
   # Estas três foram escritas contra campos que a versão final não usa.
   # A independência das fontes está verificada acima, nos campos reais.
   ('não substitui fontes', lambda d: 'Não substituir uma fonte pela outra'
      in d['meta']['regraCentral']),
   ('declara o que não permite', lambda d: len(d['oQueNaoPermite']) >= 3),
   ('estado de verificação por fonte', lambda d: all(
      'verificado' in x for x in d['serie'])),
 ],
 ...
 'data/adesao-institucional.json': [
   ...
   ('enquadramento PEDSA',  lambda d: 'enquadramentoPEDSA' in d
      and len(d['enquadramentoPEDSA']['aplicaSeA']) >= 5),
