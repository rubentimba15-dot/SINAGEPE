...
(function (global) {
  ...
  function estilo() {
    ...
    e.textContent = [
      ...
      '.ind{position:relative;background:var(--card-bg,#17243A);border:1px solid var(--card-line,rgba(72,100,144,.75));',
      ...
      'font-size:11px;line-height:1.6;color:#D6E1EE}',
      ...
      '.ind-l{font-size:9.5px;letter-spacing:.15em;text-transform:uppercase;color:var(--t4,#9DB0C8);',
      ...
      '.ind-s{font-size:10.5px;color:var(--t3,#C2D1E4);margin-top:7px;line-height:1.4}',
      '.ind-seta{font-size:9.5px;color:var(--t4,#9DB0C8);margin-top:9px}',
      '.ind.dem{border-style:dashed;border-color:rgba(255,122,122,.4)}',
      ...
      '#ind-cx{width:100%;max-width:620px;max-height:84vh;overflow-y:auto;background:#17243A;',
      'border:1px solid rgba(72,100,144,.9);border-radius:14px;box-shadow:0 20px 60px rgba(0,0,0,.55)}',
      '.ind-cab{padding:20px 22px;border-bottom:1px solid rgba(72,100,144,.75)}',
      '.ind-cab h3{margin:0 0 5px;font-size:17px;font-weight:600;color:#FAFCFF}',
      '.ind-cab p{margin:0;font-size:12.5px;color:#C2D1E4;line-height:1.6}',
      '.ind-bd{padding:16px 22px}',
      '.ind-row{display:flex;justify-content:space-between;gap:14px;padding:9px 0;',
      'border-bottom:1px solid rgba(72,100,144,.4);font-size:12px}',
      '.ind-row:last-child{border-bottom:none}',
      '.ind-row span:first-child{color:#9DB0C8}',
      '.ind-row span:last-child{color:#D6E1EE;text-align:right;font-family:"IBM Plex Mono",monospace}',
      '.ind-nota{margin:14px 22px;padding:12px 14px;border-radius:9px;font-size:11.5px;line-height:1.6;border:1px solid}',
      '.ind-nota.i{background:rgba(92,203,247,.07);border-color:rgba(92,203,247,.26);color:#9EDCF7}',
      '.ind-nota.w{background:rgba(255,195,82,.07);border-color:rgba(255,195,82,.26);color:#FFC352}',
      '.ind-nota.r{background:rgba(255,122,122,.07);border-color:rgba(255,122,122,.28);color:#FFB4B4}',
      '.ind-pe{padding:14px 22px;border-top:1px solid rgba(72,100,144,.75);font-size:10.5px;color:#9DB0C8;',
      'display:flex;justify-content:space-between;gap:14px;flex-wrap:wrap;align-items:center}',
      '.ind-pe a{color:#5CCBF7;text-decoration:none}',
      '#ind-fechar{font-family:inherit;font-size:11.5px;padding:7px 14px;border-radius:7px;',
      'border:1px solid rgba(72,100,144,.9);background:#111E31;color:#D6E1EE;cursor:pointer}'
    ].join('');
  ...
  function actualidade(o) {
    ...
    if (dias < 0) return { estado: 'impossivel', dias: dias, cor: '#BFA9FF',
      rotulo: 'data futura',
      nota: 'A data declarada \u00e9 posterior a hoje. \u00c9 erro de registo e deve ser corrigido.' };
    if (!per) return { estado: 'indeterminado', dias: dias, cor: '#9DB0C8',
    ...
    if (ciclos <= 1.5) return { estado: 'actual', dias: dias, ciclos: ciclos, cor: '#3FD9A8',
      rotulo: '\u00faltima informa\u00e7\u00e3o dispon\u00edvel', nota: base + ' Est\u00e1 dentro do intervalo esperado.' };
    if (ciclos <= 4) return { estado: 'recente', dias: dias, ciclos: ciclos, cor: '#FFC352',
      rotulo: 'com atraso', nota: base + ' Est\u00e1 atrasada cerca de ' + Math.round(ciclos) + ' ciclos.' };
    if (ciclos <= 20) return { estado: 'desfasado', dias: dias, ciclos: ciclos, cor: '#FF7A7A',
    ...
    return { estado: 'historico', dias: dias, ciclos: ciclos, cor: '#FF7A7A',
  ...
  function cartao(o) {
    ...
    var c = COR[tipo] || '#9DB0C8';
    var id = o.id || ('ind-' + Math.random().toString(36).slice(2, 9));
    REGISTO[id] = o;
    var corValor = o.cor || (tipo === 'DEMONSTRAÇÃO' ? 'var(--t3,#C2D1E4)' : 'var(--ink,#FAFCFF)');
  ...
  function abrir(id) {
    ...
    if (tipo === 'DEMONSTRAÇÃO') {
      ...
      if (o.oQueFalta) {
        h += '<div class="ind-bd">'
          + '<div class="ind-row"><span>O que falta para o calcular</span><span style="font-family:Inter,sans-serif;color:#F5DDB8">'
        ...
        h += '<div class="ind-bd"><div class="ind-row"><span>C&aacute;lculo</span><span style="font-family:Inter,sans-serif">'
          ...
          + (o.componentes || []).map(function (x) {
              return '<div class="ind-row"><span>' + x[0] + '</span><span>' + num(x[1])
                + (x[2] ? ' <span style="color:#9DB0C8">' + esc(x[2]) + '</span>' : '') + '</span></div>';
            }).join('')
          + '</div>';
      }
    ...
    h += '<div class="ind-pe"><span>'
      + (o.fonte ? '<b style="color:#C2D1E4">Fonte:</b> ' + o.fonte : 'Sem fonte declarada')
      + (o.dataDados ? ' &middot; <b style="color:#C2D1E4">Dados de:</b> ' + o.dataDados : '')
      + (function () {
          var a = actualidade(o);
          if (!a) return '';
  ...
  function faixaDemonstracao(quantos, total) {
    if (!quantos) return '';
    return '<div style="padding:13px 16px;border-radius:11px;border:1px solid rgba(255,122,122,.35);'
      + 'background:rgba(255,122,122,.07);margin-bottom:15px;font-size:12px;line-height:1.6;color:#FFB4B4">'
